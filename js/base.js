function Base() {
	...
	var bndongJs = this,
		...
		timeIds = {
			...
			setMenuScorerankTId: null, // 菜单设置-积分排名定时器ID
	...
	this.loadingAfterInit = function() {
		...
		timeIds.setMenuScorerankTId = window.setInterval(setMenuData.setScorerank, 1000);
				...
				if($('.post pre span').length > 0) {
					...
					switch(hltype) {
						case 'highlightjs':
							...
							break;
						case 'prettify':
							$('.mCSB_dragger_bar').css('background-color', $('.com').css('color'));
							break;
						case 'cnblogs':
						default:
							break;
	...
	this.setMenuData = function() {
		var introduceHtml = $('#profile_block').html(), // 个人信息
			...
			sbScorerank = $('#sidebar_scorerank ul li'), // 积分排名
